...
class CDBase
{
     ...
     * @return boolean
     */
    public static function referrerFromExternalLink()
    {
        if (empty($_SERVER['HTTP_REFERER'])) return false;
    ...
    public static function filterText($text, $defaultReplacement = '***')
    {
...
//         var_dump($keywords);exit;
        ...
        try {
            ...
            foreach ($replacement as $index => $word)
                $replacement[$index] = empty($word) ? $defaultReplacement : $word;
